...
//	LOGI_(TAG, "[+] get_remote_addr: local[%x], remote[%x]\n", local_handle, remote_handle );
	...
	while( (entry = readdir( dir )) != NULL )
	{
		...
		if ( id != 0 )
		{
			...
			if ( fp )
			{
				...
				fclose( fp );
//				LOGI_(TAG,"pid name:%s,%d",cmdline,id);
				if ( strcmp( process_name, cmdline ) == 0 )
				{
					/* process found */
//					LOGI_(TAG,"match pid name:%s,%d",cmdline,id);
					pid = id;
					break;
				}
			}
...
int inject_remote_process( pid_t target_pid, const char *library_path, const char *function_name, void *param, size_t param_size )
{
	...
	LOGI_(TAG, "[+] Remote mmap address: %x\n", (unsigned int)mmap_addr );
	...
	LOGI_(TAG, "[+] Calling mmap in target process.\n" );
	...
	LOGI_(TAG, "[+] Target process returned from mmap, return value=%x, pc=%x \n", (unsigned int)regs.ARM_r0, (unsigned int)regs.ARM_pc );
	...
	LOGI_(TAG, "[+] Get imports: dlopen: %x, dlsym: %x, dlclose: %x\n", (unsigned int)dlopen_addr, (unsigned int)dlsym_addr, (unsigned int)dlclose_addr );
	...
	LOGI_(TAG, "[+] Inject code start: %x, end: %x\n", (unsigned int)local_code_ptr, (unsigned int)&_inject_end_s );
	...
	LOGI_(TAG, "[+] _inject_function_param_s: %x\n", _inject_function_param_s );

	LOGI_(TAG, "[+] Remote shellcode address: %x\n", (unsigned int)remote_code_ptr );
...
int main(int argc, char** argv) {
	...
	pid_t target_pid;
	target_pid = find_pid_of(procName);
	if(target_pid==-1)
		return -1;
	int ret = inject_remote_process(target_pid, soPath, "InjectInterface", (void*)jarPath, strlen(jarPath) );
	LOGI_(TAG,"result: %d\n",ret);
	return ret;
}
